...
const responsesMildlySus: string[] = [
...
const responsesImpostor: string[] = [
...
export class PowCommand extends Command {
	...
	public async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
		...
		let isSus = randomIntFromInterval(1,5)
		//1 = safe
		//2 = low sus
		//3 = kinda sus
		//4 = def sus
		//5 = certainly impostor
		...
		if(isSus == 1){
			response = responsesSafe[random];		
		} else if(isSus == 2){
			response = responsesLowSus[random];
		} else if(isSus == 3){
			response = responsesMildlySus[random];
		} else if(isSus == 4){
			response = responsesVerySus[random];
		} else if(isSus == 5){
			response = responsesImpostor[random];
		}
